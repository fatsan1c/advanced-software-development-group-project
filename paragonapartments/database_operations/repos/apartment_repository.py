...
def create_occupancy_graph(parent, location=None):
    ...
    title_location = (
        location if location and location.lower() != "all" else "All Locations"
    )
    ax.set_title(
        f"Apartment Occupancy in {title_location}", fontsize=16, fontweight="bold"
    )
...
def create_performance_graph(parent, location=None):
    ...
    ax.set_title(
        f"Monthly Revenue Performance in {title_location}",
        fontsize=16,
        fontweight="bold",
    )
...
def create_apartment(
    location_ID, apartment_address, number_of_beds, monthly_rent, occupied
):
    """
